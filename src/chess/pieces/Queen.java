...
public class Queen extends ChessPiece {
	...
	public boolean[][] possibleMoves() {
		...
		// Verifica a posição que a peça vai movimentar para cima
		p.setValues(position.getRow() - 1, position.getColumn()); // O -1 esta verificando a casa acima da peça
		while (getBoard().positionExists(p) && !getBoard().thereIsAPiece(p)) {
			mat[p.getRow()][p.getColumn()] = true; // indica que a casa esta vazia e a peça pode ser movida
			p.setRow(p.getRow() - 1); // Realiza o movimento da peça para casa vazia acima
		}
		...
		// Verifica a posição que a peça vai movimentar para esquerda
		p.setValues(position.getRow(), position.getColumn() - 1); // O -1 esta verificando a casa a esquerda da peça
		while (getBoard().positionExists(p) && !getBoard().thereIsAPiece(p)) {
			mat[p.getRow()][p.getColumn()] = true; // indica que a casa esta vazia e a peça pode ser movida
			p.setColumn(p.getColumn() - 1); // Realiza o movimento da peça para casa vazia a esquerda
		}
		if (getBoard().positionExists(p) && isThereOpponentPiece(p)) { // Verifica se a casa final do movimento tem uma
																		// peça do adversario
			mat[p.getRow()][p.getColumn()] = true; // indica que a peça pode ser movida
		}

		// Verifica a posição que a peça vai movimentar para direita
		p.setValues(position.getRow(), position.getColumn() + 1); // O +1 esta verificando a casa a direita da peça
		while (getBoard().positionExists(p) && !getBoard().thereIsAPiece(p)) {
			mat[p.getRow()][p.getColumn()] = true; // indica que a casa esta vazia e a peça pode ser movida
			p.setColumn(p.getColumn() + 1); // Realiza o movimento da peça para casa vazia a direita
		}
		if (getBoard().positionExists(p) && isThereOpponentPiece(p)) { // Verifica se a casa final do movimento tem uma
																		// peça do adversario
			mat[p.getRow()][p.getColumn()] = true; // indica que a peça pode ser movida
		...
		// Verifica a posição que a peça vai movimentar para baixo
		p.setValues(position.getRow() + 1, position.getColumn()); // O +1 esta verificando a casa abaixo da peça
		while (getBoard().positionExists(p) && !getBoard().thereIsAPiece(p)) {
			mat[p.getRow()][p.getColumn()] = true; // indica que a casa esta vazia e a peça pode ser movida
			p.setRow(p.getRow() + 1); // Realiza o movimento da peça para casa vazia abaixo
		}
		if (getBoard().positionExists(p) && isThereOpponentPiece(p)) { // Verifica se a casa tem uma peça do adversario
			mat[p.getRow()][p.getColumn()] = true; // indica que a peça pode ser movida
		}
